...
function union_()
{
    ...
    st.pop();
    let c = st[st.length-1];  st.pop();
    let b = st[st.length-1];  st.pop();
    let a = st[st.length-1];  st.pop();
...
postfix_to_nfa(regexp_to_postfix(insert_concat("(0+10*)")));
        ...
        for(let i=0; i<part.length; i++) 
        {
            ...
            for(let j=0; j<2; j++) 
            {
                ...
                if(trans[0][0]!==trans[trans.length-1][0]) {
                    ...
                    for(k=1; k<trans.length&&(trans[k][0]===trans[k-1][0]); k++) {
